...
const FALLBACK_ADMIN_EMAIL = Deno.env.get('FALLBACK_ADMIN_EMAIL') || 'cegweb26@gmail.com'
...
async function sendMail(to: string, subject: string, html: string) {
  if (!RESEND_API_KEY || !to) {
    return { ok: false, skipped: true, channel: 'email', reason: 'missing_mail_config_or_recipient', to, subject }
  }
  ...
  return { ok: resend.ok, skipped: false, channel: 'email', to, subject, status: resend.status, data: raw }
}
...
async function sendSms(to: string, body: string) {
  ...
  return { ok: resp.ok, skipped: false, channel: 'sms', to: phone, data: await resp.text() }
}
...
async function sendPush(externalIds: string[], heading: string, message: string, url?: string) {
  ...
  return { ok: resp.ok, skipped: false, channel: 'push', ids, heading, message, data: await resp.text() }
}
...
function buildNotification(kind: string, payload: Record<string, unknown>, adminEmail: string) {
  ...
  const tripUrl = tripId ? `${SITE_URL.replace(/\/$/, '')}/trip.html?id=${encodeURIComponent(tripId)}` : SITE_URL
  ...
  switch (kind) {
    case 'uj_foglalas':
      ...
      sms = [{
        ...
        body: `FuvarVelünk: új foglalás érkezett a ${String(payload.indulas || '')} → ${String(payload.erkezes || '')} fuvarodra.`
      }]
      push = [{
    ...
    case 'utas_visszaigazolas':
      if (payload.utas_email || payload.email) {
        emails = [{
          ...
          html: `<h2>Sikeres foglalás</h2><p>Kedves ${passengerName}!</p><p>A foglalásod rögzítve lett:</p><p><strong>${route}</strong></p><p>Dátum: ${dateTime}</p><p>Sofőr: ${driverName}</p><p>Fizetési mód: ${payText}</p><p>Foglalt helyek: ${seats}</p><p>Kapcsolat: ${esc(payload.sofor_email || '')}</p>`
        }]
      }
      push = [{
        externalIds: [String(payload.utas_email || payload.email || '')],
        heading: 'Foglalás rögzítve',
    ...
    case 'foglalas_jovahagyva':
      ...
      sms = [{
        ...
        body: `FuvarVelünk: a foglalásodat jóváhagyták. ${String(payload.indulas || '')} → ${String(payload.erkezes || '')}, ${String(payload.datum || '')} ${String(payload.ido || '')}.`
      }]
      push = [{
        externalIds: [String(payload.utas_email || payload.email || '')],
    ...
    case 'fizetve_jelolve':
      if (payload.utas_email || payload.email) {
        emails = [{
          ...
          html: `<h2>Fizetés visszaigazolva</h2><p>Kedves ${passengerName}!</p><p>A foglalásod fizetése rögzítve lett.</p><p><strong>${route}</strong></p><p>Dátum: ${dateTime}</p><p>Sofőr: ${driverName}</p>`
        }]
      }
      push = [{
        externalIds: [String(payload.utas_email || payload.email || '')],
    ...
    case 'utas_indulas_emlekezteto':
      if (payload.utas_email || payload.email) {
        emails = [{
          to: String(payload.utas_email || payload.email),
          subject: `Indulási emlékeztető: ${String(payload.indulas || '')} → ${String(payload.erkezes || '')}`,
          html: `<h2>Indulási emlékeztető</h2><p>Kedves ${passengerName}!</p><p>A foglalt fuvarod 2 órán belül indul.</p><p><strong>${route}</strong></p><p>Dátum: ${dateTime}</p><p>Sofőr: ${driverName}</p><p>Kapcsolat: ${esc(payload.sofor_email || '')}${payload.sofor_telefon ? ' · ' + esc(payload.sofor_telefon) : ''}</p>`
        }]
      }
      sms = [{
        to: String(payload.utas_telefon || payload.telefon || ''),
        body: `FuvarVelünk: 2 órán belül indul a foglalt utad. ${String(payload.indulas || '')} → ${String(payload.erkezes || '')}, ${String(payload.datum || '')} ${String(payload.ido || '')}.`
      }]
      push = [{
        externalIds: [String(payload.utas_email || payload.email || '')],
        heading: '2 órán belül indul az utad',
        message: `${String(payload.indulas || '')} → ${String(payload.erkezes || '')} · ${String(payload.datum || '')} ${String(payload.ido || '')}`,
        url: tripUrl
      }]
      break
    ...
    default:
      ...
      push = [{
        ...
        message: `${String(payload.indulas || '')} → ${String(payload.erkezes || payload.cel || '')}`,
        url: SITE_URL
      }]
      break
  }
...
async function writeLog(kind: string, payload: Record<string, unknown>, subject: string, results: unknown[]) {
  try {
    ...
    await admin.from('email_naplo').insert([{
      ...
      cel_email: recipient,
      statusz: emailResult?.ok || pushResult?.ok ? 'elkuldve' : (emailResult?.skipped && pushResult?.skipped ? 'kihagyva' : 'sikertelen'),
      sikeres: !!(emailResult?.ok || pushResult?.ok),
...
serve(async (req) => {
  ...
  try {
    ...
    const anyOk = results.some((x: any) => x?.ok)
    return new Response(JSON.stringify({
      ok: anyOk,
      subject: mainSubject,
      results,
