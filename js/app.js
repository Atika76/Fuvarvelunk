...
const App = (() => {
  ...
  async function fetchApprovedTrips(filters={}) {
    let q = sb.from(tableTrips).select('*').eq('status','Jóváhagyva').order('datum',{ascending:true}).order('ido',{ascending:true});
  ...
  function tripCard(trip, admin=false) {
    ...
    const paymentMethods = (trip.fizetesi_modok && Array.isArray(trip.fizetesi_modok) ? trip.fizetesi_modok : ['barion','cash']).map(m => m === 'cash' ? 'Készpénz a helyszínen' : 'Barion').join(' · ');
    return `
      <article class="card trip-card" data-trip-id="${trip.id}">
        <div class="trip-main">
          <div class="inline-pills"><span class="pill">${escapeHtml(trip.indulas)} → ${escapeHtml(trip.erkezes)}</span>${statusBadge(trip.status || 'Jóváhagyva')}</div>
        ...
        <div class="trip-actions">
          ...
          ` : `
            ...
            <a class="btn btn-secondary" href="kapcsolat.html">Kapcsolat</a>
          `}
        </div>
      </article>`;
  }
    ...
    return `
      <article class="card admin-item">
        <div>
          ...
          <p><strong>Fizetési mód:</strong> ${b.fizetesi_mod === 'cash' ? 'Készpénz a helyszínen' : 'Barion'}</p>
      ...
      if (bookBtn) {
        ...
        const wrap = openModal(`
          ...
          <form id="bookingForm" class="form-stack">
            ...
            <div class="notice warn">Bankkártyás fizetéshez a Barion szerveroldali aktiválása még szükséges. Készpénzes foglalásnál a rendszer azonnal lefoglalja a helyet.</div>
        ...
        wrap.querySelector('#bookingForm').addEventListener('submit', async ev => {
          ...
          try {
            ...
            msg.textContent = booking.fizetesi_mod === 'cash' ? 'Sikeres foglalás. A hely lefoglalva.' : 'Foglalás rögzítve. A Barion fizetés szerveroldali aktiválása még szükséges.';
